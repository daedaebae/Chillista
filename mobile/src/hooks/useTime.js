...
export const useTime = (initialState) => {
    ...
    const advanceTime = useCallback((minutes) => {
        return new Promise((resolve) => {
            setTimeState(prev => {
                if (prev.paused || !prev.gameStarted) return prev;
                if (prev.minutesElapsed >= 480) { // 5:00 AM + 480 mins = 1:00 PM ? Wait, 8 hours. 5 to 13.
                    // The logic in useGame said 480. 
                    return prev;
                }
                ...
                resolve(newState); // Return state to promise for other hooks? 
                // Actually, hooks don't work like that. The useEffect in GameLoop drives this.
                // We'll just return the state update.
                return newState;
            });
        });
    }, []);
